...
def validate_developer_tester_neighbors(workplace):
    ...
    if not any(skill in new_employee_skills for skill in ['Тестирование', 'Бэкенд', 'Фронтэнд']):
    ...
    if left_workplace and left_workplace.employee:
        ...
        left_is_developer = any(skill in left_skills for skill in ['Бэкенд', 'Фронтэнд'])
        
        print(f"Левый сосед - тестировщик: {left_is_tester}, разработчик: {left_is_developer}")
        ...
        if is_tester and left_is_developer:
            ...
            raise ValidationError('Нельзя размещать тестировщика слева от разработчика!')
        ...
        if is_developer and left_is_tester:
            ...
            workplace.employee = old_employee  # Возвращаем старое состояние
            raise ValidationError('Нельзя размещать разработчика слева от тестировщика!')
    ...
    if right_workplace and right_workplace.employee:
        ...
        right_is_developer = any(skill in right_skills for skill in ['Бэкенд', 'Фронтэнд'])
        
        print(f"Правый сосед - тестировщик: {right_is_tester}, разработчик: {right_is_developer}")
        ...
        if is_tester and right_is_developer:
            ...
            workplace.employee = old_employee  # Возвращаем старое состояние
            raise ValidationError('Нельзя размещать тестировщика справа от разработчика!')
        ...
        if is_developer and right_is_tester:
            ...
            workplace.employee = old_employee  # Возвращаем старое состояние
            raise ValidationError('Нельзя размещать разработчика справа от тестировщика!')
